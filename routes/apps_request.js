...
const schema_name = "2983EF607EE549399932EE9B472B0415";
const table_name  = "7414942E8CBC4E1E90238C15FF62BBE2";

const column_001_shorttext = "001_shorttext";
const column_002_shorttext = "002_shorttext";
const column_003_shorttext = "003_shorttext";
...
router.post('/', (req, res) => {
    ...
    //get json, example {"text1": "hogehoge", "text2": "hogehoge","text3": "hogehoge",}
    ...
    try{
        jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, function(err, decoded) {
            ...
            pool.query(`INSERT INTO \"${schema_name}\".\"${table_name}\" ("ID", \"${column_001_shorttext}\", \"${column_002_shorttext}\",\"${column_003_shorttext}\") VALUES ($1, $2, $3, $4)`,[id, text1, text2, text3], (err, results) => {
                if(err){
                    logger.error(err);
                    return res.status(400).json(err.toString());
                }
...
router.patch('/:postId', (req, res)=>{
    ...
    try{
        jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, function(err, decoded) {
            ...
            pool.query(`UPDATE \"${schema_name}\".\"${table_name}\" SET \"${column_001_shorttext}\" = \'${text}\' WHERE "ID" = \'${postId}\'`, (err, results) => {
                if(err){
                    logger.error(err);
                    return res.status(400).json(err.toString());
                }
